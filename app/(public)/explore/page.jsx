...
export default function ExplorePage() {
  ...
  const handleViewLocalEvents = () => {
    const city = currentUser?.location?.city || "Bhopal";
    const state = currentUser?.location?.state || "Madhya Pradesh";
            ...
            <CarouselContent>
              {featuredEvents.map((event) => (
                <CarouselItem key={event._id}>
                  ...
                  <div
  className="relative h-[250px] sm:h-[300px] md:h-[350px] rounded-xl overflow-hidden cursor-pointer"
  onClick={() => handleEventClick(event.slug)}
>
  {event.coverImage ? (
    <Image
  ...
  <div className="absolute bottom-0 left-0 w-full p-4 sm:p-6 md:p-12">
    <Badge className="w-fit mb-2 sm:mb-4" variant="secondary">
      {event.city}, {event.state || event.country}
    </Badge>

    <h2 className="text-xl sm:text-2xl md:text-5xl font-bold mb-2 text-white">
      {event.title}
    </h2>

    <p className="text-sm sm:text-base text-white/90 mb-3 line-clamp-2 sm:line-clamp-3">
      {event.description}
    </p>
